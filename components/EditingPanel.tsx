...
const EditingPanel: React.FC<EditingPanelProps> = ({
  ...
  isGeneratingDocx,
  storyPrompt,
  generateContent, // Add this new prop
}) => {
  const handleContinueStory = async (pageIndex: number, selectedText: string) => {
    if (activeLanguage !== defaultLanguage) {
      setActiveLanguage(defaultLanguage);
      toast(`Switched to ${defaultLanguage} for content generation.`);
    }
    
    try {
      const newContent = await generateContent(pageIndex, selectedText);
      
      // Update the page content with the new generated text
      const currentVersion = languageVersions.find(v => v.language === defaultLanguage);
      if (currentVersion && currentVersion.pages[pageIndex]) {
        const updatedPage = { ...currentVersion.pages[pageIndex] };
        const lastTextBlockIndex = updatedPage.blocks.findIndex(block => block.type === 'text');
        if (lastTextBlockIndex !== -1) {
          updatedPage.blocks[lastTextBlockIndex].content += '\n\n' + newContent;
          updatePageContent(defaultLanguage, pageIndex, updatedPage);
        }
      }
    } catch (error) {
      console.error('Error generating content:', error);
      toast.error('Failed to generate new content. Please try again.');
    }
  };
